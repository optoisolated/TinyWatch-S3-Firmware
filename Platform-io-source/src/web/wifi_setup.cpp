...
void WifiSetup::start()
{
	...
	webServer.on("/", HTTP_GET, [](AsyncWebServerRequest *request) { request->send(200, "text/html", String(startHtml) + scanningHtml); }).setFilter(isPortalRequest);
	...
	webServer.on("/connectresult", HTTP_GET, [this](AsyncWebServerRequest *request) { request->send(request->beginChunkedResponse("text/html", connect_responder(this))); }).setFilter(isPortalRequest);
